...
sealed class ApiState<ResultType> {
    ...
    companion object {
         ...
         */
        fun <ResultType> loading(): ApiState<ResultType> =
            Loading()

        /**
         * Creates [ApiState] object with [Success] state and [data].
         */
        fun <ResultType> success(data: ResultType): ApiState<ResultType> =
            Success(data)

        /**
         * Creates [ApiState] object with [Error] state and [ErrorType].
         */
        fun <ResultType> error(errorType: ErrorType): ApiState<ResultType> =
            Error(errorType)
    }
}